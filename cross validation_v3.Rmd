...
# TWFE + shape cross validation
```{r}
# covariates
covars = c("fit_gauss_a", "fit_gauss_b", "fit_gauss_depth", "fit_gauss_sigmasq","thg_coeff_0","thg_coeff_2","thg_coeff_4")
```
...
```{r}
# get rv dataframe
current_df = standardize(final_df, covars, test_index, RESPONSE_CHANGE = F, TRAIN_TEST_SPLIT = T) %>%
  mutate(date = factor(date),
         line_order = factor(line_order)) %>%
  select(rv_template_0.5, line_order, date, fit_gauss_a_centered, fit_gauss_b_centered, fit_gauss_depth_centered, fit_gauss_sigmasq_centered, thg_coeff_0_centered, thg_coeff_2_centered, thg_coeff_4_centered)
```
...
```{r}
trainAndTest = standardize(current.df, covars, test_index = dayIndex, TRAIN_TEST_SPLIT = T)
...
```{r}
# hg fit model
currentlm = lm(rv_template_0.5 ~ date + line_order + fit_gauss_a_centered + fit_gauss_b_centered + fit_gauss_depth_centered + fit_gauss_sigmasq_centered + thg_coeff_0_centered + thg_coeff_2_centered + thg_coeff_3_centered + thg_coeff_4_centered + thg_coeff_5_centered + thg_coeff_6_centered + thg_coeff_7_centered, train.df )
```
...
Xtest = model.matrix( ~ line_order + fit_gauss_a_centered + fit_gauss_b_centered + fit_gauss_depth_centered + fit_gauss_sigmasq_centered + thg_coeff_0_centered + thg_coeff_2_centered + thg_coeff_3_centered + thg_coeff_4_centered + thg_coeff_5_centered + thg_coeff_6_centered + thg_coeff_7_centered, test.df)
```
