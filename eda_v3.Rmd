...
final_lams = ( final_df %>% group_by(lambdac_vald) %>% summarize(n. = n()) )$lambdac_vald
length(final_lams)
```

```{r}
# list of lambda-orders in final_df
final_lams_orders = ( final_df %>% group_by(line_order) %>% summarize(n. = n()) )$line_order
...
```{r}
final_df %>%
  #filter(date %in% final_days[1:15]) %>%
  ggplot() +
...
```{r}
pert.df %>%
  filter(line_order %in% sample( final_lams_orders, 2)) %>%
  ggplot(mapping = aes(x = date, y = rv_template_0.5_change, color = line_order) ) +
  geom_point(alpha = .2) +
  geom_line(alpha = .5)
```
