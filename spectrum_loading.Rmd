...
lineProp_list = list.files("line_property_files")
...
spectra_df = loadSolarFile(solarspectrum_list[1])
```
...
# example line
line_list[1]
line_df = loadLineFile( line_list[1] )
```
...
```{r}
line_df %>%
  filter(date %in% all_dates[c(1,101,202)]) %>%
  ggplot() +
...
lineProp_list[1]
single_lineprop = loadLineShape(lineProp_list[1])
```
