...
plot_df = rv_df %>%
  ...
  select(date, date_groups, lineID, planet_rv, fit_gauss_depth, change_depth)
```

```{r}
seq_days = seq(min(unique(plot_df$date)), max(unique(plot_df$date)), by = 1)

# planet df for the smooth curve
...
rv_time_plt = plot_df %>%
  ggplot() +
  geom_point(mapping = aes(x = date, y = planet_rv, color = date), size = .8, alpha = .87) +
...
rv_depth_plt = plot_df %>%
  ggplot() +
  geom_point(mapping = aes(x = change_depth, y = planet_rv, color = date ), size = .8, alpha = .87) +
  guides(color="none") +
...
print(final_plot)
ggsave("visuals/fourLinesRV_planet.png", width = 13, height = 10)
``` 
...
# print the final arrangement
print(final_plot)
ggsave("visuals/fourLinesRV.png", width = 13, height = 10)
``` 
...
print(final_plot)
ggsave(filename = "visuals/heatmapRV_combined.png", width = 8, height = 7, units = "in")
