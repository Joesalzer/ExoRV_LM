...
```{r}
covar.list = c( str_c("hg_",c(0:12)),"snr", "a1", "depth1", "width1", "b1", "rv")
covar.list = c(covar.list, str_c( covar.list, "_center" ))
...
```{r}
covar.list[covar.list != "hg_1"]
```

```{r}
covar = "hg_1"

for (var in covar.list[covar.list != covar] ) {
  print(
    model.df %>%
      drop_na() %>%
      ggplot() +
      geom_point(mapping = aes(x = !!sym(covar), y = !!sym(var), color = wavelength))
  )
...
model.df %>%
  filter(wavelength<8600) %>%
  ggplot() +
  geom_point(mapping = aes(x = depth1, y = hg_1, color = wavelength),alpha = .2)
```



```{r}
rm(covar.list, covar, var)
```
...
model.df %>%
  group_by(obs_date) %>%
  summarize( mean_rv_center = mean(rv_center) ) %>%
  ungroup() %>%
  mutate(date = as.Date(obs_date)) %>%
  ggplot() +
  geom_point(mapping = aes(x = date, y = mean_rv_center))
```
mean change in rv over time ~2 se bars

recall that the sd for a given day would be 
```{r}
model.df %>%
  group_by(obs_date) %>%
  summarize( mean_rv_center = mean(rv_center),
             sd_error = sd(rv_center)/sqrt( length(final_lams_orders) ) ) %>%
  ungroup() %>%
  mutate(date = as.Date(obs_date)) %>%
  ggplot() +
  geom_point(mapping = aes(x = date, y = mean_rv_center)) +
  geom_errorbar(aes(x = date, ymin = mean_rv_center - 1.96*sd_error, ymax = mean_rv_center + 1.96*sd_error), color = "black")
```



```{r}
quantile(mean_rv_center, .005)
```
...
model.df %>%
  ...
  group_by(obs_date) %>%
  summarize( mean_rv_center = mean(rv_pert_center),
             sd_error = sd(rv_pert_center)/sqrt( length(final_lams_orders) ) ) %>%
  ungroup() %>%
  mutate(date = as.Date(obs_date)) %>%
  ggplot() +
  geom_point(mapping = aes(x = date, y = mean_rv_center))
