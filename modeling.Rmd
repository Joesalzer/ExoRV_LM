...
below are special types of the linear models, common slopes for covariateNames, and multiple intercept/slope per lineID/time group models
...
```{r}
cv_list <- pbmclapply(left_out_timeIDs, FUN = cv_parallel_LASSO, mc.cores = 7)
#cv_df = do.call(rbind, cv_list)
rm(cv_list)
```
...
for (i in 1:length(lambdas)) {
  ...
  # initialize 0's in the linear operator
  linear_op_mat = matrix(data = 0, nrow = T_, ncol = length(fit_lm$beta_hat[,1]) )
  # matrix for estimating the cleaned RV
  linear_op_mat[,(length(group_sizes)+1):sum(group_sizes)] = contr_groupSum(group_sizes)[,length(group_sizes):(sum(group_sizes)-1)]
  # dataframe of date's cleaned rv
  cleanRV_df = data.frame(
...
```{r}
saveRDS(list(lambdas = lambdas,
             numZeroCols = numZeroCols,
             AICs = AICs,
             BICs = BICs,
             RMSEs = RMSEs,
             RSEs = RSEs),
        file = str_c(model_dir, "/LASSO_results.rds") )
```
...
saveRDS(list(designMat = designMat_LASSO,
        ...
        file = str_c(model_dir, "/model_LASSO.rds" ) )
```
