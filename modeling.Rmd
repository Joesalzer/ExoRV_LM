...
```{r}
lm0 = lm(centered_rv ~ 0 + b1*lambda + obs_date + order_idx, scaled_final_fits )
```
...
```{r}
datecoef.df = tibble( 
                obs_date = names(lm0$coefficients[ startsWith( names(lm0$coefficients), "obs_date") ]),
                coef_og = lm0$coefficients[ startsWith( names(lm0$coefficients), "obs_date") ]
              ) %>%
...
```{r}
lambdacoef.df = tibble( 
                lambda = names(lm0$coefficients[ startsWith( names(lm0$coefficients), "lambda") ]),
                coef_og = lm0$coefficients[ startsWith( names(lm0$coefficients), "lambda") ]
              ) %>%
              mutate(lambda = factor(str_sub(lambda,7)))
lambdacoef.df %>%
  ggplot() +
  geom_point(mapping = aes(x = lambda, y = coef_og)) +
  theme(axis.text.x = element_blank(),
        axis.ticks = element_blank() )
```
...
```{r}
rm(lm0, datecoef.df, lambdacoef.df)
```
...
lm.test = lm(centered_rv ~ lambda + order_idx + obs_date, data = scaled_final_fits )
```
...
for (day in final_days) {
  ...
  # linear model on training data
  lm.train = lm(rv ~ lambda*b1 + order_idx + obs_date + b1 + gh_3 + gh_2 + gh_5 + 
    gh_7 + depth1 + gh_9 + a1 + snr + gh_11 + gh_4 + width1 + 
    gh_0 + gh_10, train )
...
tail.df = pred.df %>% filter(pred_error < err.quantiles[1] | pred_error > err.quantiles[5])
```
...
123 (out of 245) lines appear in the tail of the distribution
60 (out of 69) orders appear in the tail of the distribution
...
Which lambdas appear in tail more than 200 times:
```{r}
bad_lams = ( tail.df %>% group_by(lambda) %>% summarize(n = n()) %>% filter(n > 200) )$lambda
bad_lams 
```
...
Let's remove the 3 "bad" lambdas and see how much we improve our model:
...
*compared ~roughly~ to 4.473138 and 2.454458*
