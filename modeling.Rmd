...
```{r}
lm0 = lm(rv_center ~ lam_order*b1_center + obs_date , model.df)
```
...
## model for rv by hg1*line_order

```{r}
# the current linear model to be worked on
currentlm = lm(rv_center ~ 0 + hg_1_center + lam_order*hg_1_center, model.df %>% filter(wavelength < 8600, wavelength != 5175.169) )
```
...
```{r}
model.df %>%
  filter(wavelength < 8600, wavelength != 5175.169) %>%
...
```{r}
model.df = model.df %>% mutate(day = as.numeric(  as.Date(obs_date) ) )
model.df
```

```{r}
# the current linear model to be worked on
currentlm = lm(rv_center ~ obs_date + I(obs_date^2) , model.df)
```
