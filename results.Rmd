...
Gauss=all_THG=all
Gauss=depth_HG=all
Gauss=all_HG=0,2,3,4,5,6,7
Gauss=all_HG=0,2,3,4,5,6,7,8
...
Gauss=all_HG=0,2,3,4,5,6,7 
 ...
 RSE: 7.4127
 Parameters: 9665
 RMSE of cleaned RV to true planet: 0.689 
 mean se: 0.27 
 coverage 1se: 0.297
 coverage 2se: 0.582
...
Gauss=all_HG=0,2,3,4,5,6,7,8 
 ...
 RSE: 7.2328
 Parameters: 10443
 RMSE of cleaned RV to true planet: 0.66 
 mean se: 0.264 
 coverage 1se: 0.309
 coverage 2se: 0.594
...
Gauss=all_HG=0,2,3,4,5,6,7,8,9 
 ...
 RSE: 7.0385
 Parameters: 11221
 RMSE of cleaned RV to true planet: 0.636 
 mean se: 0.257 
 coverage 1se: 0.33
 coverage 2se: 0.588
...
ggsave("visuals/cleanRV.png", width = 10, height = 5)
```
...
ggsave("visuals/se_cleanRV.png", width = 10, height = 5)
```
```{r}
...
ggsave("visuals/CI_coverage.png", width = 7, height = 7)
```
...
cv_df %>%
  ...
  ggplot() +
  geom_point(mapping = aes(x = as.Date(date), y = seRV, color = modelID), size = 1, alpha = .85) +
  theme_minimal() +
  scale_y_log10() +
  scale_color_manual(values=c("#ff7f00", "#377eb8")) +
...
ggsave("visuals/cv_se.png", width = 10, height = 5)
```
